...
void PrintFeatureScores(const classifier::subgradient::SubgradientHinge& sh,
  ...
  for (classifier::feature_vector::const_iterator it = train[0].fv.begin();
    ...
    sh.CompareFeatureWeight(word, &results);
...
int main(int argc, char** argv) {
  ...
  classifier::subgradient::SubgradientHinge sh;
