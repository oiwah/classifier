...
void PrintFeatureScores(const classifier::perceptron::AveragedPerceptron& av_perc,
  ...
  for (classifier::feature_vector::const_iterator it = train[0].fv.begin();
    ...
    av_perc.CompareFeatureWeight(word, &results);
...
int main(int argc, char** argv) {
  ...
  PrintFeatureScores(av_perc, train);
